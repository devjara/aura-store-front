...
export class MenComponent implements OnInit {
  ...
  public categories = signal<CategoryWithCount[]>([]);
  public isLoading = signal<boolean>(true);
  public skeletons = [1, 2, 3, 4, 5];

  // ─── Tabs de productos ────────────────────────────────────────────────────────
  public productTabs = ['Todos', 'Camisas', 'Pantalones', 'Outerwear', 'Accesorios', 'Calzado'];
  public activeProductTab = signal<string>('Todos');

  // ─── Tabs de diseñadores ──────────────────────────────────────────────────────
  public designerTabs = ['Todos', 'Vireon', 'Cavren', 'Noctra', 'Althen'];
  public activeDesignerTab = signal<string>('Todos');

  // ─── Productos computados ─────────────────────────────────────────────────────
  public featuredProducts = computed(() => this.products().filter(p => p.category.toLowerCase() === 'hombres').slice(0, 10));
  public designerProducts = computed(() => this.products().filter(p => p.category.toLowerCase() === 'hombres').slice(0, 10));
  ...
  async ngOnInit() {
    ...
    if (isPlatformBrowser(this.platformId)) {
      try {
        ...
        // Filtra solo categorías de hombre y agrega conteo real
        this.categories.set(
          categories
